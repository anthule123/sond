...
class Medical {
  ...
  // nội dung chung cho phuong an DTD Type 2 khong tiem Insulin
  String glucose_infusion_6H12H18H22H = """ 
-	 Bổ sung 1UI novorapid cho mỗi 15g CHO
"""; //  6h - 12h - 18h – 22h
  get getGlucoseinfusion_6H12H18H22H => this.glucose_infusion_6H12H18H22H;
  set setGlucoseinfusion_6H12H18H22H(String value) =>
      this.glucose_infusion_6H12H18H22H = value;
  void setInitialGlucoseinfusion_6H12H18H22H(value2) =>
      this.glucose_infusion_6H12H18H22H = """ 
...
//-	Tiêm dưới da insulin tác dụng chậm
  String yInsu22H = """ 
- Tiêm dưới da insulin tác dụng chậm :
    + liều khởi đầu: 0.2 UI/ngày hoặc liều đang tiêm tại nhà hoặc 50% tổng liều insulin truyền tĩnh mạch/24h gần nhất
...
// nội dung phương án bổ sung khi không đạt mục tiêu
  ...
  void _setSloveFailedContext(int value) =>
      this.sloveFailedContext = "-  Bổ sung ${value} UI novorapid";
// nội dung phương án bổ sung khi không đạt mục tiêu ở lộ trình cuối
  String sloveLastFailedContext = "-  Tiêm dưới da Actrapid 2UI";
  get getLastSloveFailedContext => this.sloveLastFailedContext;
  void _setLastSloveFailedContext(int value) =>
      this.sloveFailedContext = "-  Tiêm dưới da ${value} UI novorapid";
...
// nội dung phương án cho không đạt mục tiêu ở phương án sử dụng trên
  String upLatium = """+ Tăng liều 10 - 20%
                       + Glargine/ Detemir 1 lần/24h hoặc NPH 2 lần/ ngày cách nhau 12h""";
  ...
  // Kiểm tra nồng độ Glucozo theo dõi 4-6h/ 1 lan
  void set_Content_State_Check_Last_Gluco_Failed(double gluco) {
    if ((8.3 < gluco) && (gluco <= 11.1)) {
      _setLastSloveFailedContext(4);
    } else if (gluco > 11.1) {
      _setLastSloveFailedContext(6);
    } else {
      setYInsu22H(0.2);
      print("Có lỗi xảy ra, không xác định");
    }
  ...
  void setChangeStatus() {
    if (!checkBreak) {
      if (!this.checkCurrentGlucose) {
        ...
        if (getCheckOpenCloseTimeStatus("5:30", "6:30")) {
        ...
        } else if (getCheckOpenCloseTimeStatus("11:30", "12:30")) {
          this._content_display =
              "Bạn phải đợi đến 12h  để đo đường máu mao mạch";
        } else if (getCheckOpenCloseTimeStatus("17:30", "18:30")) {
          if (this.checkDoneTask) {
            this._content_display =
          ...
          this._content_display =
              "Bạn phải đợi đến 18h để đo đường máu mao mạch";
        } else if (getCheckOpenCloseTimeStatus("21:30", "22:30")) {
          if (this.checkDoneTask) {
            this._content_display =
  ...
  bool checkValidMeasuringTimeFocus() {
    if (getCheckOpenCloseTimeStatus('5:30', '6:30') ||
        getCheckOpenCloseTimeStatus('11:30', '12:30') ||
        getCheckOpenCloseTimeStatus('17:30', '18:30') ||
        getCheckOpenCloseTimeStatus('21:30', '22:30')) {
      return true;
    }
    return false;
  }
  ...
  // Hiện đo CHO
  bool isVisibleCarbon = false;
  void setChangeVisibleCarbon() => this.isVisibleCarbon = !this.isVisibleCarbon;
  ...
  void addLabelDatatoListHistoryFailed() {
    if (this._lastStateBool) {
      ...
      listHistoryTimeInjection
          .add("Phương án tăng liều Glargine/Determir 10-20%");
